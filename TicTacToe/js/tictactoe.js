...
function placeXOrO(squareNumber) {
  ...
  if (!selectedSquares.some((element) => element.includes(squareNumber))) {
    ...
    } //Active player may only be 'X' or 'O' so, if not 'X' it mmust be 'O'
    else {
    ...
    //This condition checkes to see if it is the computers turn.
    if (activePlayer === "O") {
    ...
    //Returning true needed for our computerTurn() function to work.
    return true;
  }
...
//drawLine() function is called to draw a Line in the screen if the condition is met.
function checkWinConditions() {
  ...
  //This condition checks for tie. If non of the above conditions are met and
  // 9 squares are selected the code executes.
  else if (selectedSquares.length >= 9) {
    //This function plays the tie game sound.
    audio('./media/tie.mp3');
    //This function sets a .3 second timer befire the resetGame is called.
    setTimeout(function () {
      resetGame();
    }, 500);
  }
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
  ...
  // This line cals our main animation Loop.
  animateLineDrawing();
